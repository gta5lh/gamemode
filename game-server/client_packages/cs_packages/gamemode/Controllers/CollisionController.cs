...
namespace GamemodeClient.Controllers
{
	...
	public class CollisionController : Events.Script
	{
		...
		{
			if (entity.Type != RAGE.Elements.Type.Vehicle || !Player.IsInVehicle() || entity.Id != Player.CurrentPlayer.Vehicle.Id)
			{
				return;
			}
			...
			if (Entities.Vehicles != null && Entities.Vehicles.Count > 0)
			{
				...
				{
					if (!vehicle.Exists || vehicle.Id == target.Id)
					{
						continue;
					}
		...
		public void OnEntityStreamIn(RAGE.Elements.Entity entity)
		{
			...
			if (!Player.IsInVehicle() || Player.CurrentPlayer.Vehicle.GetSharedData(DataKey.VehicleCollisionDisabled) == null || !(bool)Player.CurrentPlayer.Vehicle.GetSharedData(DataKey.VehicleCollisionDisabled))
			{
				return;
			}

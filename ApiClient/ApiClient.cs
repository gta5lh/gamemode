...
namespace Gamemode.ApiClient
{
    public static class ApiClient
    {
        ...
        public static async Task<List<Report>> AllAwaitingReports()
        {
            HttpResponseMessage httpResponseMessage = await client.GetAsync($"http://localhost:8000/v1/reports/awaiting");
            ...
            return JsonConvert.DeserializeObject<List<Report>>(response);
        }
        ...
        public static async Task AnswerReport(ReportAnswer request)
        {
            string json = JsonConvert.SerializeObject(request);
            StringContent data = new StringContent(json, Encoding.UTF8, "application/json");

            HttpResponseMessage httpResponseMessage = await client.PostAsync($"http://localhost:8000/v1/reports/{request.ReportId}/answer", data);

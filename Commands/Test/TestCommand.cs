...
namespace Gamemode.Commands.Test
{
    public class TestCommand : Script
    {
        ...
        [Command("getcachedreports")]
        [AdminMiddleware(AdminRank.Owner)]
        public async void GetCachedReports(CustomPlayer player)
        {
            foreach (ReportCacheEntity reportCacheEntity in ReportCache.GetAll())
            {
                player.SendChatMessage($"{reportCacheEntity.Report.Question}");
            }
        }
    }
}
